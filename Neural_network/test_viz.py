...
vec = np.load("results/CNN_n_epoch_5_batch_random_with_rpl_alg_GD_lr_4.0_momentum_0.0_seed_42_convexity_test_results.npy",allow_pickle=True)
...
size_vec = len(np.load(root + list_net[i] + setting  + "SGD" + list_param[0] + list_seed[0] + "_convexity_test_results.npy", allow_pickle=True).item()["convexity_diff_list"])
size_vec_det = len(np.load(root + list_net[i] + setting  + "GD" + list_param[3] + list_seed[0] + "_convexity_test_results.npy", allow_pickle=True).item()["convexity_diff_list"])
...
for j in range(len(list_seed)):
    dict_racoga_sgd = np.load(root + list_net[i] + setting  + "SGD" + list_param[0] + list_seed[j] + "_convexity_test_results.npy", allow_pickle=True)
    dict_racoga_snag = np.load(root + list_net[i] + setting  + "SNAG" + list_param[1] + list_seed[j] + "_convexity_test_results.npy", allow_pickle=True)
    dict_racoga_nag = np.load(root + list_net[i] + setting  + "NAG" + list_param[2] +list_seed[j] +"_convexity_test_results.npy", allow_pickle=True)
    dict_racoga_gd = np.load(root + list_net[i] + setting  + "GD" + list_param[3] +list_seed[j] +"_convexity_test_results.npy", allow_pickle=True)
    racoga_trajectory_sgd += np.array(dict_racoga_sgd.item()["convexity_diff_list"])
    racoga_trajectory_snag += np.array(dict_racoga_snag.item()["convexity_diff_list"])
    racoga_trajectory_nag += np.array(dict_racoga_nag.item()["convexity_diff_list"])
    racoga_trajectory_gd += np.array(dict_racoga_gd.item()["convexity_diff_list"])
...
if include_RMSprop == True  and include_adam == True:
    ...
    labelpad_x = 2
# plt.subplot(121)
nb_gd_eval_det = torch.arange(0,782*5  ,782)
col = (0.3, 0.6, 1 - 1*0.8/3)
plt.plot(racoga_trajectory_sgd[:3911],label = "SGD",c = "black",lw=2.5)
plt.plot(racoga_trajectory_snag[:3911],label = "SNAG",c = col,lw=2.5)

m_loss = np.min(np.array([racoga_trajectory_sgd[:3911].min(),racoga_trajectory_snag[:3911].min(),racoga_trajectory_nag[:5].min(),racoga_trajectory_gd[:5].min()]))
m_loss = round(m_loss,1) 
plt.plot(racoga_trajectory_nag[:5],label = "NAG",c = "red",lw=3)
plt.plot(racoga_trajectory_gd[:5],label = "GD",c = "purple",lw=3,linestyle="--")
...
# m = round(m,1)
# m_sto = np.max(np.array([(racoga_trajectory_gd[:5]).max(),(racoga_trajectory_nag[:6]).max()]))
# m_sto = round(m_sto,1)
print(racoga_trajectory_nag)
print(racoga_trajectory_sgd)
...
if include_adam == True and include_RMSprop == False:
    ...
    plt.savefig("figures/convexity_measure.png")
